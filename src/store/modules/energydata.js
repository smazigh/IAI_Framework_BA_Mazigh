...
const actions = {
    ...
    async fetchApiData({ commit }) {
        ...
        for (let index = 0; index < state.apiData.data.length; index++) {
            dataStructured.push({ id: new Date(state.apiData.data[index].year, state.apiData.data[index].month, state.apiData.data[index].day), dayContent: state.apiData.data[index] });
